...
exports.checklistDemAbsUser = (req, res, next) => {
  Abs.find({ $and: [{ userId: req.user }, { ...req.body }] })
    .then((demande) => {
      if (!demande) return res.status(409).json({ error: "No request !" });
...
exports.checklistDemAbs = async (req, res, next) => {
  Abs.find({ ...req.body })
    .populate({
      path: "userId",
      populate: { path: "managId", select: "Nom Prenom" },
      select: "Nom Prenom",
    })
    .then((dem) => {
      if (!dem) {
...
exports.checklistDemCgGroup = async (req, res, next) => {
  ...
  for (let i = 0; i < users.length; i++) {
    let d = await Abs.find({
      $and: [{ userId: users[i]._id }, { ...req.body }],
    });
    if (!d) return res.status(401).json({ error: "Cette section est vide !" });
    t[i] = {
      Nom: users[i].Nom,
