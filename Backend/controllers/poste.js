...
exports.createPoste = async (req, res) => {
  ...
  if (!dep) {
    return res.status(409).json("Departement n existe pas");
  }
...
exports.editPost = async (req, res) => {
  ...
  if (req.body.dep) {
    Dep.findOne({ NomD: req.body.dep }).then((dep) => {
  ...
  post.findOneAndUpdate(
    ...
    (err) => {
      if (err) {
        res
          .status(400)
          .json({ error: "Modification ne peut etre sauvegardee" });
      } else {
