...
export default function Update() {
  ...
  const handleSubmit = async (e) => {
    ...
    const Com = await axios.put(
      `http://localhost:4000/conge/edit/${id}`,
      form,
      config
    );
        ...
        <form action="#">
          <div className="postupform">
            <div className="colone">
              <label htmlFor="nomP">Date Debut</label>
              <input
                type="date"
                name="nomP"
                defaultValue={form.DateDebut}
                onChange={(e) => setForm({ DateDebut: e.target.value })}
              />

              <label htmlFor="Salaire">Date Fin</label>
              <input
                type="date"
                name="Salaire"
                defaultValue={form.DateFin}
                onChange={(e) => setForm({ DateFin: e.target.value })}
              />
              <label htmlFor="Salair">Status</label>
              <textarea
                type="text"
                name="Salair"
            ...
            {!error && open && (
              ...
              >
                <DialogTitle id="alert-dialog-title">
                  {" Planification modifie "}
                </DialogTitle>
                <DialogContent>
                  <DialogContentText id="alert-dialog-description">
                    La Planification a bien été modifie .
                  </DialogContentText>
                </DialogContent>
                <DialogActions>
                  <Button onClick={() => navigate(`/ListCongA`)}>
                    {" "}
                    Retour{" "}
                  </Button>
                </DialogActions>
