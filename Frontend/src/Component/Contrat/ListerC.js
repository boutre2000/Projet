  ...
  { headerName: 'Actions',
    ...
    getActions: (params) => [
      <GridActionsCellItem icon={<RiIcons.RiEditLine style={{color: '#2D3436'}}/>}   onClick={()=>
        navigate(`/UpdateCont/${params.id}`)
      }  label="Modifier" showInMenu />,
      <GridActionsCellItem icon={<RiIcons.RiEditLine style={{color: '#2D3436'}}/>}   onClick={(e)=>
        telecharger(e,params.id)
      }  label="Voir Support" showInMenu />
...
React.useEffect(()=>{
  const fetchFCT = async()=>{
      try{
          ...
          const {data} = await axios.get('http://localhost:4000/contrat/list', config)
          
          data.map(item=>{
            
        item.dateEd= dayjs(data?.dateEd)?.format('YYYY-MM-DD')
        
        
        item.userId= item?.posteId?.userId?.Nom +' '+ item?.posteId?.userId?.Prenom
...
const telecharger= (e,id) =>{
  e.preventDefault();
  axios.get(`http://localhost:4000/contrat/viewpj/${id}`, config, {responseType: 'blob'})
  .then(dem=>{
  ...
  return (
    ...
    <button className='congebtna' onClick={()=>
        navigate(`/listcu`)}> Mes contrats</button>
    <button className='congebtn' onClick={()=>
        navigate(`/AjoutCont`)}> Nouveau contrat</button>
