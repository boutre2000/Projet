...
export default function AjoutC() {
  ...
  const send = event => {
    ...
    axios.post('http://localhost:4000/contrat/add', data, config)
      .then(res => 
        {
  ...
  return (
    ...
    <form action="#">
      ...
      <label  htmlFor="email">Employé</label>
        <select name="email" defaultValue=""   onChange={e =>setEmail(e.target.value)}   >
        <option hidden value="" style={{color:'#B9C4C8'}}>--Choisir--</option>
        ...
        <select name="postId" defaultValue="" onChange={e => setNomP(e.target.value)}   >
        <option hidden value="" style={{color:'#B9C4C8'}}>--Choisir--</option>
                       {datad.map((d, index)=>(
                        <option key={index} >
                        {d.nomP}
         ...
         <div className="ficont">
            <label  className="postupfi" htmlFor="file">{pj} <GrIcons.GrUpload/> </label>
            <input
              className="fiinp"
              type="file"
              id="file"
