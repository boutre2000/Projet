...
import lottieJson from "./103620-success.json";
...
export default function UpdatePM() {
    ...
    const fetchFCT = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:4000/post/preview/${id}`,
          config
        );
        setForm((curr) => ({
  ...
  const send = async (e) => {
    ...
    const post = await axios.put(
      `http://localhost:4000/post/edit/${id}`,
      formu,
      config
    );
        ...
        <form action="#">
          <div className="postupform">
            <div className="colone">
              ...
              <select
                name="Situation"
                defaultValue={form.Situation}
                onChange={(e) => setFormu({ Situation: e.target.value })}
              >
            ...
            <div className="coltwo">
              ...
              <select
                name="StatusP"
                defaultValue={form.StatusP}
                onChange={(e) => setFormu({ StatusP: e.target.value })}
              >
            ...
            <div className="colthree">
              ...
              <select
                name="userId"
                defaultValue={form.userId?.email}
                onChange={(e) => setFormu({ emp: e.target.value })}
              >
                {datap.map((d, index) => (
